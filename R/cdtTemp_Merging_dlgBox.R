...
mrgGetInfoTemp <- function(){
    ...
    conf.tab1 <- bwAddTab(bwnote, text = "Input Data")
    conf.tab2 <- bwAddTab(bwnote, text = "Merging Parameters")
    ...
    frTab1 <- tkframe(conf.tab1)
        ...
        txt.InNCDF <- tklabel(frInputData, text = 'Directory containing the temperature data', anchor = 'w', justify = 'left')
        ...
        helpWidget(en.InNCDF, 'Enter the full path to directory containing the temperature data files', 'Enter the full path to directory containing the temperature data files')
        ...
        helpWidget(en.outmrgff, 'Format of the merged filenames in NetCDF, example: tmax_mrg_1983011.nc', 'Format of the merged filenames in NetCDF, example: tmax_mrg_1983011.nc')
    ...
    frTab2 <- tkframe(conf.tab2)
        ...
        bt.mrg.interp <- ttkbutton(frTab2, text = "Merging Interpolations Parameters")
        ...
        bt.grid.interp <- ttkbutton(frTab2, text = "Create Grid for Interpolation")

        helpWidget(bt.grid.interp, 'Create the grid to interpolate the data', 'Create the grid to interpolate the data')
        ...
        chk.blank <- tkcheckbutton(frBlank, variable = blank.data, text = "Blank merged data using ESRI shapefiles", anchor = 'w', justify = 'left')
    ...
    tkconfigure(bt.prm.OK, command = function(){
        if(str_trim(tclvalue(file.stnfl)) == ""){
            ...
            cdt.tkmessageBox(tt, message = "Browse or enter the directory containing the temperature data", icon = "warning", type = "ok")
            tkwait.window(tt)
            ...
            cdt.tkmessageBox(tt, message = "No shapefile provided", icon = "warning", type = "ok")
            tkwait.window(tt)
        }else{
