...
preview.data.nc <- function(parent.win, openncf){
	...
	OK.but <- ttkbutton(fr.button, text = .cdtEnv$tcl$lang$global[['button']][['1']], width = 8)
	CA.but <- ttkbutton(fr.button, text = .cdtEnv$tcl$lang$global[['button']][['2']], width = 8)
	...
	for(i in seq_along(print.nc)) tkinsert(txta, "end", paste(print.nc[i], "\n"))
	tcl("update")
