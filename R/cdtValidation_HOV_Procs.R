...
HandOutValidationDataProcs <- function(GeneralParameters){
		...
		!isTRUE(all.equal(.cdtData$EnvData$opDATA$AggrSeries, AggrSeries)))
	{
		...
		if(GeneralParameters$aggr.series$aggr.data){
			...
			agg.index <- cdt.index.aggregate(.cdtData$EnvData$opDATA$dates, timestep,
											out.series, seasonLength, startMonth)
	...
	Insert.Messages.Out('Calculate statistics ...')
	...
	.cdtData$EnvData$Statistics$STN <- ValidationStatisticsFun(AggrcdtData, AggrncdfData, dichotomous)
	matSTN <- matrix(AggrcdtData, ncol = 1)
	matCDF <- matrix(AggrncdfData, ncol = 1)
	.cdtData$EnvData$Statistics$ALL <- ValidationStatisticsFun(matSTN, matCDF, dichotomous)
	matSTN <- matrix(rowMeans(AggrcdtData, na.rm = TRUE), ncol = 1)
	matCDF <- matrix(rowMeans(AggrncdfData, na.rm = TRUE), ncol = 1)
	.cdtData$EnvData$Statistics$AVG <- ValidationStatisticsFun(matSTN, matCDF, dichotomous)
	...
	Insert.Messages.Out('Statistics calculation done!')
