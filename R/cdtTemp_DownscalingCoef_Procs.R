...
Temp_execCoefDown <- function(){
	origdir <- file.path(.cdtData$GalParams$IO.files$dir2save, paste0('CoefDownTemp', '_',
						file_path_sans_ext(.cdtData$GalParams$IO.files$STN.file)))

	dir.create(origdir, showWarnings = FALSE, recursive = TRUE)
	Insert.Messages.Out('Computing regression parameters ...')
	...
	.cdtData$GalParams$paramsGlmCoef <- list(stnData = stnData, demData = demData, origdir = origdir)
...
Temp_CoefDownscaling <- function(){
	Insert.Messages.Out('Compute downscaling coefficients ...')
	...
	allyears <- .cdtData$GalParams$Down.Date.Range$all.years
	year1 <- .cdtData$GalParams$Down.Date.Range$start.year
	year2 <- .cdtData$GalParams$Down.Date.Range$end.year
	minyear <- .cdtData$GalParams$Down.Date.Range$min.year
	...
	Insert.Messages.Out('Computing downscaling coefficients finished')
	rm(stnData, demData)
	gc()
	return(0)
}
