...
getParams.TT.OutlierCheck <- function(Parameters)
{
    ...
    tkconfigure(bt.prm.OK, command = function(){
        Parameters$temp.max <<- as.numeric(trimws(tclvalue(temp.max)))
        Parameters$temp.min <<- as.numeric(trimws(tclvalue(temp.min)))
        Parameters$window <<- as.numeric(trimws(tclvalue(window)))
