...
split.polygons.with_missing <- function(x, y1, y2){
	...
	if(!is.null(ixy)){
		...
		if(xna$values[length(xnl)]){
			y1l <- head(y1, -xnl[length(xnl)])
			y2l <- head(y2, -xnl[length(xnl)])
			ilu <- length(y1l)
		...
		if(xna$values[length(xnl)]){
			x <- head(x, -xnl[length(xnl)])
			y1 <- head(y1, -xnl[length(xnl)])
			y2 <- head(y2, -xnl[length(xnl)])
		}
...
split.polygons.gradients.with_missing <- function(polys, breaks){
	...
	POLY <- lapply(seq_along(polys), function(j){
		pl <- Polygon(do.call(cbind, polys[[j]][c('x','y')]))
		Polygons(list(pl), nom.pl[j])
	})
	POLY <- SpatialPolygons(POLY, seq_along(polys))
	...
	LINE <- lapply(seq_along(breaks), function(j){
		sl <- Line(cbind(xcrd, breaks[j]))
		Lines(list(sl), nom.sl[j])
	})
	LINE <- SpatialLines(LINE)
