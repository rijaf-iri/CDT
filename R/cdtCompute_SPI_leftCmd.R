...
SPICalcPanelCmd <- function(){
	...
	subfr1 <- bwTabScrollableFrame(cmd.tab1)
		...
		tkconfigure(calculateBut, command = function(){
			...
			Insert.Messages.Out("Calculate SPI ......")
	...
	subfr2 <- bwTabScrollableFrame(cmd.tab2)
		...
		chk.dataIdx <- tkcheckbutton(frameDataExist, variable = .cdtData$EnvData$DirExist, text = "SPI data already computed", anchor = 'w', justify = 'left')
		...
		tkconfigure(bt.spi.maps, command = function(){
				...
				!is.null(.cdtData$EnvData$varData))
			{
				...
				SPICalc.Display.Maps('SPI - Map', 'SPI - Time Series')
			}
		})
	...
	subfr3 <- bwTabScrollableFrame(cmd.tab3)
		...
		tkconfigure(bt.VizTS, command = function(){
			if(!is.null(.cdtData$EnvData$varData)){
				...
				imgContainer <- CDT.Display.Graph(SPICalc.Plot.VizTS, .cdtData$EnvData$tab$spiViz, 'SPI - Time Scales')
	...
	get.Data.Map <- function(){
		...
		if(.cdtData$EnvData$output$params$data.type == "cdtstation"){
			...
			if(readVarData){
				...
				nc.file <- file.path(.cdtData$EnvData$PathData, "DATA_NetCDF", tscale.data, paste0("spi_", this.daty, ".nc"))
				nc <- nc_open(nc.file)
