...
execBiasTemp <- function(){
    ...
    Insert.Messages.Out('Computing bias coefficients ...', TRUE, "i")
    ...
    ncInfoBias <- ncInfo.no.date.range(.cdtData$GalParams$TEMP,
    ...
    bias.pars <- ComputeBiasCoefficients(stnData = stnData, ncInfo = ncInfoBias,
                                         params = .cdtData$GalParams, variable = "temp",
                                         outdir = outdir)
    ret <- InterpolateBiasCoefficients(bias.pars, xy.grid, variable = "temp",
