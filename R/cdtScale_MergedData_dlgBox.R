...
Merging_ScaleDataInfo <- function(){
	...
	file.period <- tclVar()
	CbperiodVAL <- .cdtEnv$tcl$lang$global[['combobox']][['1']][2:4]
	periodVAL <- c('daily', 'pentad', 'dekadal')
	tclvalue(file.period) <- CbperiodVAL[periodVAL %in% .cdtData$GalParams$mrg.data$tstep]

	txtdek <- switch(.cdtData$GalParams$mrg.data$tstep, 'dekadal' = 'Dekad', 'pentad' = 'Pentad', 'Day')
	day.txtVar <- tclVar(txtdek)

	cb.period <- ttkcombobox(frDate, values = CbperiodVAL, textvariable = file.period, width = largeur1)
	bt.DateRange <- ttkbutton(frDate, text = "Set Date Range")

	tkconfigure(bt.DateRange, command = function(){
		.cdtData$GalParams[["Scaling.Date"]] <- getInfoDateRange(.cdtEnv$tcl$main$win,
												.cdtData$GalParams[["Scaling.Date"]],
	...
	tkconfigure(set.mrgData, command = function(){
		.cdtData$GalParams[["mrg.data"]] <- getInfoNetcdfData(tt, .cdtData$GalParams[["mrg.data"]],
														str_trim(tclvalue(dir.mrgData)), str_trim(tclvalue(file.period)))
	})
	...
	cb.scalefun <- ttkcombobox(frScaleFun, values = c("sum", "mean"), textvariable = scale.fun, width = 5)
