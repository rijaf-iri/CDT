...
spatialAnalysisProcs <- function(GeneralParameters){
    ...
    if(!dir.exists(GeneralParameters$out.dir)){
        ...
        Insert.Messages.Out(paste('The outputs will be put in', getwd()), TRUE, "w")
    ...
    outputDIR <- file.path(GeneralParameters$out.dir, paste0("SPATIAL.ANALYSIS_",
                            ...
                            tools::file_path_sans_ext(basename(GeneralParameters$in.file))))
    ...
    tstep.in <- GeneralParameters$in.tstep
    tstep.out <- GeneralParameters$time.series$out.series
    ...
    if(tstep.out == "annual" & startMonth != 1){
        seasonLength <- 12
        tstep.out <- "seasonal"
    }
    ...
    agg.index <- cdt.index.aggregate(daty[itmp], tstep.in, tstep.out, seasonLength = seasonLength, startMonth = startMonth)
    ...
    if(aggregatData){
        ...
        Insert.Messages.Out("Aggregation done!", TRUE, "s")
