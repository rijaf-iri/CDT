...
getInfo_volumetricValid <- function(parent.win, statData, Parameters, obsOnly = FALSE){
    ...
    xml.dlg <- file.path(.cdtDir$dirLocal, "languages", "cdtValidation_volumetricStat_dlgBox.xml")
    ...
    tkbind(chk.user, "<Button-1>", function(){
        ...
        if(statData == "stn"){
            ...
            if(tclvalue(unique.thres) == '0'){
                tkconfigure(widgetIN$cb.stnfl, state = stateUser)
                tkconfigure(widgetIN$bt.stnfl, state = stateUser)
            }else{
        ...
        if(tclvalue(userVal) == '1'){
            stateBaseP <- if(statData == "all") 'disabled' else 'normal'
    ...
    widgetIN$cb.stnfl <- NULL
    widgetIN$bt.stnfl <- NULL
    ...
    multipleThresFun <- function(){
        txt.stnfl <- tklabel(frameThres, text = lang.dlg[['label']][['2']], anchor = 'w', justify = 'left')
        widgetIN$cb.stnfl <- ttkcombobox(frameThres, values = unlist(listOpenFiles), textvariable = file.stnfl, width = largeur0, state = stateUser)
        widgetIN$bt.stnfl <- tkbutton(frameThres, text = "...", state = stateUser)

        tkgrid(txt.stnfl, row = 0, column = 0, sticky = 'we', rowspan = 1, columnspan = 10, padx = 1, pady = 0, ipadx = 1, ipady = 1)
        tkgrid(widgetIN$cb.stnfl, row = 1, column = 0, sticky = 'we', rowspan = 1, columnspan = 9, padx = 0, pady = 0, ipadx = 1, ipady = 1)
        tkgrid(widgetIN$bt.stnfl, row = 1, column = 9, sticky = 'we', rowspan = 1, columnspan = 1, padx = 0, pady = 0, ipadx = 1, ipady = 1)

        helpWidget(widgetIN$cb.stnfl, lang.dlg[['tooltip']][['3']], lang.dlg[['status']][['3']])
        helpWidget(widgetIN$bt.stnfl, lang.dlg[['tooltip']][['4']], lang.dlg[['status']][['4']])

        ##help
        #####
        tkconfigure(widgetIN$bt.stnfl, command = function(){
            ...
            if(!is.null(dat.opfiles)){
                ...
                tkconfigure(widgetIN$cb.stnfl, values = unlist(listOpenFiles))
            }
        })
    }
    ...
    file.stnfl <- tclVar(Parameters$user.file)
    ...
    if(statData == "stn"){
        ...
        frameThres <- tkframe(fr.user)
        uniqueThresFun()
        tkgrid(frameThres, row = 0, column = 0, sticky = 'we', padx = 1, pady = 1, ipadx = 1, ipady = 1)
    }
