...
Validation.STAT.PanelCmd <- function(){
	...
	GeneralParameters <- fromJSON(file.path(.cdtDir$dirLocal, 'init_params', 'Validation_STAT.json'))
	...
	##############################################
		...
		CbCLIMVARS <- c("Precipitation", "Temperature")
		climvarVAL <- c('RR', 'TT')
		climVAR <- tclVar()
		tclvalue(climVAR) <- CbCLIMVARS[climvarVAL %in% GeneralParameters$clim.var]
		...
		txt.climvar <- tklabel(frInputData, text = 'Variable', anchor = 'e', justify = 'right')
		cb.climvar <- ttkcombobox(frInputData, values = CbCLIMVARS, textvariable = climVAR)

		txt.stnfl <- tklabel(frInputData, text = 'Station data used to validate', anchor = 'w', justify = 'left')
		cb.stnfl <- ttkcombobox(frInputData, values = unlist(listOpenFiles), textvariable = file.stnfl1, width = largeur)
		...
		cb.valid <- ttkcombobox(frInputData, values = unlist(listOpenFiles), textvariable = file.stnfl2, width = largeur)
		...
		tkgrid(txt.climvar, row = 1, column = 0, sticky = 'we', rowspan = 1, columnspan = 2, padx = 1, pady = 2, ipadx = 1, ipady = 1)
		tkgrid(cb.climvar, row = 1, column = 2, sticky = 'we', rowspan = 1, columnspan = 3, padx = 1, pady = 2, ipadx = 1, ipady = 1)
		...
		infobulle(cb.stnfl, 'Select the file containing the station data from the list')
		status.bar.display(cb.stnfl, 'Select the file containing the station data used to validate in CDT format')
		infobulle(bt.stnfl, 'Browse file if not listed')
		status.bar.display(bt.stnfl, 'Browse file if not listed')

		infobulle(cb.valid, 'Select the file containing the station data from the list')
		status.bar.display(cb.valid, 'Select the file containing the data to be validated in CDT format')
		infobulle(bt.valid, 'Browse file if not listed')
		status.bar.display(bt.valid, 'Browse file if not listed')
	...
	##############################################
		...
		if(str_trim(tclvalue(climVAR)) == 'Precipitation') trhesVal <- 1
		if(str_trim(tclvalue(climVAR)) == 'Temperature') trhesVal <- 20
		...
		tkconfigure(bt.calc.stat, command = function(){
			...
			if(getInputDATA){
				...
				if(retNULL){
					Insert.Messages.Out(msg1, format = TRUE)
					return(NULL)
				}
			}
	...
	getInputInfos <- function(){
		...
		GeneralParameters$clim.var <- climvarVAL[CbCLIMVARS %in% str_trim(tclvalue(climVAR))]
