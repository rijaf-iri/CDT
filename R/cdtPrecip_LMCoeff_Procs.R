...
execLMCoefRain <- function(){
	...
	.cdtData$GalParams$lmCoefParms <- list(model.coef = model.coef, months = months,
					...
					ncInfo = ncInfo, LMCoef.DIR = origdir,
					interp.grid = list(grid = xy.grid, nlon = nlon0, nlat = nlat0))
	ret <- Precip_InterpolateLMCoef()

	rm(model.coef, ncInfo, stnData, demData, rfeDataInfo)
	gc()
	if(!is.null(ret)){
		if(ret == 0) return(0)
		else return(ret)
...
Precip_InterpolateLMCoef <- function(){
