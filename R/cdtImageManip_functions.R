...
tkrplot.win <- function(parent, fun, hscale = 1, vscale = 1){
	...
	png(filename = tmpfl, width = 385 * hscale, height = 385 * vscale, restoreConsole = FALSE)
...
tkrreplot.win <- function(lab, fun = lab$fun, hscale = lab$hscale, vscale = lab$vscale){
	tmpfl <- paste0(tempfile(), '.png')
	on.exit(unlink(tmpfl))
	png(filename = tmpfl, width = 385 * lab$hscale, height = 385 * lab$vscale, restoreConsole = FALSE)
...
tkrplot1 <- function(...) if(WindowsOS()) tkrplot.win(...) else tkrplot(...)

tkrreplot1 <- function(...){
	if(WindowsOS()){
		ret <- tkrreplot.win(...)
	}else{
		ret <- try(tkrreplot(...), silent = TRUE)
		if(inherits(ret, "try-error"))
			Insert.Messages.Out(gsub('[\r\n]', '', ret[1]), format = TRUE)
		ret <- if(inherits(ret, "try-error")) NULL else 0
	}
	return(ret)
}

refreshPlot1 <- function(W, img, hscale, vscale){
	ret <- tkrreplot1(img, hscale = hscale, vscale = vscale)
...
LatLonLabels <- function(xlon, xlat){
	...
	degsym <- if(WindowsOS()) "\xb0 " else  "° "
	lon_lab <- paste0(sprintf("%03d", degLo), degsym, sprintf("%02d", minLo), "' ",
				paste0(sprintf("%02d", trunc(secLo)), '.', substr(sprintf("%.2f", frac(secLo)), 3, 4)), '" ', sLon)
	lat_lab <- paste0(sprintf("%03d", degLa), degsym, sprintf("%02d", minLa), "' ",
				paste0(sprintf("%02d", trunc(secLa)), '.', substr(sprintf("%.2f", frac(secLa)), 3, 4)), '" ', sLat)
...
getAdminLabel <- function(xyMouse, parPltCrd = NULL, shp, idField){
	...
	if(tclvalue(tkwinfo('exists', idField$ID)) == "1"){
		admin_name <- admin_name[as.integer(tclvalue(tcl(idField, 'current'))) + 1]
