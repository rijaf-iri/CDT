...
CPT.parse.Date.Filename <- function(dirNC, filename){
	...
	all.files <- list.files(dirNC, paste0("*.", file_ext(filename), "$"))
	ymdpt <- c("Y", "M", "D", "P", "T")
...
CPT.convertGridData.Files <- function(ncInfo, output.file, cptInfo = NULL){
	...
	nc.varid <- ncInfo$varid
	lon <- nc$var[[nc.varid]]$dim[[ncInfo$xo]]$vals
	lat <- nc$var[[nc.varid]]$dim[[ncInfo$yo]]$vals
	...
	for(jj in seq_along(daty)){
		...
		nc_close(nc)
		xdon <- if(ncInfo$xo < ncInfo$yo) xdon[xo, yo] else t(xdon)[xo, yo]
