                                        ...
                                        outdir, demData, GUI = TRUE)
{
    ...
    if(bias.method %in% c("mbvar", "mbmon")){
        ...
        if(interp.method %in% c("idw", "okr", "nns")){
            ...
            coordinates(locations.bs) <- ~lon+lat
        }
    ...
    if(bias.method == "qmdist"){
        if(variable == "rain"){
            PROB <- ncvar_def("prob", "", list(dx, dy), NA,
                              longname = "Probability of non-zero event Bernoulli-Gamma distribution",
                              prec = "float", compression = 9)
            SCALE <- ncvar_def("scale", "", list(dx, dy), NA,
                               longname = "Scale parameters of the gamma distribution",
                               prec = "float", compression = 9)
            SHAPE <- ncvar_def("shape", "", list(dx, dy), NA,
                               longname = "Shape parameters of the gamma distribution",
                               prec = "float", compression = 9)
            ncoutPARS <- list(PROB, SCALE, SHAPE)
            nbvar <- 3
            distrname <- "BernGamma"
        }

        if(variable == "temp"){
            MEAN <- ncvar_def("mean", "degC", list(dx, dy), NA,
                              longname = "Means normal distribution",
                              prec = "float", compression = 9)
            SD <- ncvar_def("sd", "degC", list(dx, dy), NA,
        ...
        if(interp.method %in% c("idw", "okr", "nns")){
            ...
            locations.stn <- as.data.frame(bias.pars$data[c('slon', 'slat')])
...
applyBiasCorrection <- function(bias, ncInfo, outdir, params, variable, GUI = TRUE){
    ...
    dy <- ncdim_def("Lat", "degree_north", bias$lat)
    xy.dim <- list(dx, dy)
    grd.bsadj <- ncvar_def(varinfo$name, varinfo$units, xy.dim, varinfo$missval,
