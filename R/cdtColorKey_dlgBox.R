...
createColorkey <- function(parent.win, listCol){
	...
	if(length(listCol) > 0){
		listCol <- rgb(t(col2rgb(listCol, alpha = TRUE)), maxColorValue = 255)
		nlCol <- length(listCol)
		varCol <- tclVar(listCol[nlCol])
	}else varCol <- tclVar('red')

	listCanColor <- list()
	kcol <- if(WindowsOS()) 10 else 14
	...
	parseKolor <- function(vcolor){
		...
		if(length(txtcol) > 0){
			...
			existcol <- vtxtcol %in% colors()
			if(any(!existcol)){
				Insert.Messages.Out(paste('Unknown color name:', paste0(vtxtcol[!existcol], collapse = ", ")), format = TRUE)
				vcolor <- vcolor[-txtcol[!existcol]]
				if(length(vcolor) == 0){
					Insert.Messages.Out('No valid color name', format = TRUE)
					return(NULL)
				}
			}
		}
	...
	txtCol_width <- if(WindowsOS()) 30 else 40
	...
	tkadd(popMenuRemColor, "command", label = "Change", command = function(){
		...
		if(nchar(color) > 0){
			...
			for(icvs in seq_along(listCol)) tkinsert(textColor, "end", paste0(listCol[icvs], ', '))
		}
	})
	...
	labColor <- tklabel(frCmd, text = 'Color:')
	canColor <- tkcanvas(frCmd, width = 80, height = 25, bg = tclvalue(varCol))
	addColor <- tkbutton(frCmd, text = "Add")
	saveColor <- tkbutton(frCmd, text = "Save")
	loadColor <- tkbutton(frCmd, text = "Load")
	...
	frEditCmd <- tkframe(frDialog, relief = 'groove', bd = 2)

	editColor <- tclVar(0)

	chkButColor <- tkcheckbutton(frEditCmd, variable = editColor, text = 'Edit Colors', anchor = 'w', justify = 'left')
	dispButColor <- tkbutton(frEditCmd, text = "Show")
	...
	dispGradColor <- tkbutton(frPreviewCmd, text = "Preview")
	...
	tkgrid(frCmd, row = 0, column = 0, columnspan = 3, sticky = 'w')
	tkgrid(frListColor, row = 1, column = 0, columnspan = 3, sticky = 'w')
	tkgrid(frEditCol, row = 2, column = 0, columnspan = 3)
	tkgrid(frEditCmd, row = 3, column = 0, columnspan = 2, sticky = 'we')
	tkgrid(frPreviewCmd, row = 3, column = 2, columnspan = 1, sticky = 'e')
	tkgrid(frPreviewCanvas, row = 4, column = 0, columnspan = 3)
	...
	tkconfigure(bt.opt.OK, command = function(){
		...
		listCol <<- if(!is.null(listCol)) rgb(t(col2rgb(listCol, alpha = TRUE)), maxColorValue = 255) else NULL
