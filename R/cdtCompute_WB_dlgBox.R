...
computeWB_getParams <- function(){
    ...
    tkconfigure(bt.prm.OK, command = function(){
        if(str_trim(tclvalue(input.Etp)) %in% c("", "NA")){
            cdt.tkmessageBox(tt, message = "No input for daily potential evapotranspiration", icon = "warning", type = "ok")
            tkwait.window(tt)
        }else if(str_trim(tclvalue(input.Prec)) %in% c("", "NA")){
            cdt.tkmessageBox(tt, message = "No input for daily precipitation", icon = "warning", type = "ok")
            tkwait.window(tt)
...
computeWB_get.WB.SWHC <- function(parent.win, Parameters, dataType, donne)
{
    ...
    tkconfigure(bt.prm.OK, command = function(){
        if(str_trim(tclvalue(input.file)) %in% c("", "NA")){
            cdt.tkmessageBox(tt, message = "No input data found", icon = "warning", type = "ok")
            tkwait.window(tt)
        }else{
