...
Format_CDT_Input_Station_Data <- function(){
	...
	file.period <- tclVar()
	cb.periodVAL <- .cdtEnv$tcl$lang$global[['combobox']][['1']][c(2, 4, 5)]
...
multipleFileCDTFormat <- function(top.win, tstep.list, tstep){
	...
	largeur <- if(WindowsOS()) 33 else 26
	...
	ffrmt2 <- tkradiobutton(fr.fileformat1, text = lang.dlg[['radiobutton']][['1']][2], anchor = 'w', justify = 'left')
...
singleFileCDTFormat <- function(top.win, tstep.list, tstep){
	listOpenFiles <- openFile_ttkcomboList()
	largeur <- if(WindowsOS()) 38 else 30
