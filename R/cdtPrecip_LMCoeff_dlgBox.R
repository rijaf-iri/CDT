...
coefLMGetInfoRain <- function(){
	...
	# xml.dlg <- file.path(.cdtDir$dirLocal, "languages", "cdtPrecip_LMCoeff_dlgBox.xml")
	...
	frTab1 <- tkframe(conf.tab1)
		...
		infobulle(en.RFE, 'Enter the full path to the directory containing the RFE or Adjusted RFE files')
		status.bar.display(en.RFE, 'Enter the full path to the directory containing the RFE or Adjusted RFE files')
	...
	frTab2 <- tkframe(conf.tab2)
		...
		status.bar.display(frGrid, 'Create the grid to interpolate the LM coefficients')
