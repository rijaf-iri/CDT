...
ExecDownload_DEM <- function(){
	...
	if(inherits(ret, "try-error")){
		...
		if(ret == 0) Insert.Messages.Out(.cdtData$GalParams[['message']][['4']])
		else Insert.Messages.Out(.cdtData$GalParams[['message']][['5']], format = TRUE)
	}
	...
	if(inherits(ret1, "try-error")){
		...
		if(ret1 == 0) Insert.Messages.Out(.cdtData$GalParams[['message']][['6']])
		else Insert.Messages.Out(.cdtData$GalParams[['message']][['7']], format = TRUE)
	}
	return(0)
}
