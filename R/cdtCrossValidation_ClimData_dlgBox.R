...
crossValidationInfoClimData <- function(){
    ...
    if(.cdtData$GalParams$action == "crossv.rain"){
        ...
        txt.rnor.wet <- tklabel(frRnoR, text = lang.dlg[['label']][['16']], anchor = 'e', justify = 'right')
    ...
    ############################################
        ...
        crossvalid.stations <- function(selstn){
            ...
            if(selstn == "file"){
                ...
                txt.datatype <- tklabel(fr.datatype, text = lang.dlg[['label']][['12']], anchor = 'e', justify = 'right')
                ...
                tkbind(cb.datatype, "<<ComboboxSelected>>", function(){
                    if(str_trim(tclvalue(selstn.filetype)) == CbdatatypeVAL[1])
                        tclvalue(txt.INData.var) <- lang.dlg[['label']][['13']]
                ...
                txt.INData <- switch(.cdtData$GalParams$selstn$file.type,
                                     'cdtcoords' = lang.dlg[['label']][['13']],
                                     'cdtstation' = lang.dlg[['label']][['14']]
                                    )
            ...
            if(selstn == "cdt"){
                txt.minperc <- tklabel(frstnPar, text = lang.dlg[['label']][['15']], anchor = 'w', justify = 'left')
    ...
    txt.selstn <- tklabel(frSTN, text = lang.dlg[['label']][['11']], anchor = 'e', justify = 'right')
