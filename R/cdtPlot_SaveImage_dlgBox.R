...
SavePlot <- function(){
    ...
    cb.img.unit <- ttkcombobox(frB, values = c("px", "in", "cm", "mm"), textvariable = uImage, width = 5)

    txt.img.res <- tklabel(frB, text = lang.dlg[['label']][['4']], anchor = 'e', justify = 'right')
    en.img.res <- tkentry(frB, textvariable = rImage, width = 6, state = stateRes)
    ...
    en.img.width <- tkentry(frB, textvariable = wImage, width = 6)
    ...
    en.img.height <- tkentry(frB, textvariable = hImage, width = 6)
