...
SeasonAnalysisPanelCmd <- function(){
	...
	if(WindowsOS()){
		...
		largeur2 <- .cdtEnv$tcl$fun$w.widgets(27)
		largeur3 <- 26
		largeur4 <- 15
		largeur5 <- 14
		largeur6 <- 22
	...
	subfr2 <- bwTabScrollableFrame(cmd.tab2)
		...
		tkconfigure(bt.CalcPICSA, command = function(){
			...
			Insert.Messages.Out("Calculate rainy season data ......")
	...
	read.PicsaTSData <- function(){
		...
		if(.cdtData$EnvData$output$data.type == "cdtstation"){
			...
			if(rasterTsData){
				...
				tmp <- cdt.as.image(tmp, nx = nx, ny = ny, pts.xy = cbind(.cdtData$EnvData$output$data$lon, .cdtData$EnvData$output$data$lat))
	...
	calculate.ClimStat <- function(){
		...
		if(.cdtData$EnvData$output$data.type == "cdtstation"){
			...
			if(calcClim){
				...
				nx <- nx_ny_as.image(diff(range(.cdtData$EnvData$output$data$lon)))
				ny <- nx_ny_as.image(diff(range(.cdtData$EnvData$output$data$lat)))
				don <- cdt.as.image(don, nx = nx, ny = ny, pts.xy = cbind(.cdtData$EnvData$output$data$lon, .cdtData$EnvData$output$data$lat))
			...
			if(calcClim){
				...
				toExports <- ".cdtData$EnvData"
				packages <- c("stringr", "tcltk", "doParallel")
	...
	statisticFunction <- function(don){
		...
		if(str_trim(tclvalue(.cdtData$EnvData$analysis.method)) == 'Average'){
			don <- colMeans(don, na.rm = TRUE)
		}

		if(str_trim(tclvalue(.cdtData$EnvData$analysis.method)) == 'Median'){
			don <- matrixStats::colMedians(don, na.rm = TRUE)
		}

		if(str_trim(tclvalue(.cdtData$EnvData$analysis.method)) == 'Standard deviation'){
			don <- matrixStats::colSds(don, na.rm = TRUE)
		}

		if(str_trim(tclvalue(.cdtData$EnvData$analysis.method)) == 'Trend'){
			tmp <- TrendFunction(don, start.dateYear)
			if(str_trim(tclvalue(.cdtData$EnvData$trend)) == "Change (trend) / year") don <- tmp
			if(str_trim(tclvalue(.cdtData$EnvData$trend)) == "Change (trend) over the period"){
		...
		if(str_trim(tclvalue(.cdtData$EnvData$analysis.method)) == 'Percentiles'){
			Q <- as.numeric(str_trim(tclvalue(.cdtData$EnvData$mth.perc))) / 100
			don <- apply(don, 2, quantile8, probs = Q)
		}

		if(str_trim(tclvalue(.cdtData$EnvData$analysis.method)) == 'Frequency'){
			xlow <- str_trim(tclvalue(.cdtData$EnvData$low.thres))
			xup <- str_trim(tclvalue(.cdtData$EnvData$up.thres))
			if(str_trim(tclvalue(.cdtData$EnvData$varPICSA)) %in% c("Onset", "Cessation")){
				...
				xlow <- as.numeric(as.Date(dlo) - .cdtData$EnvData$output$start.date)
				xup <- as.numeric(as.Date(dup) - .cdtData$EnvData$output$start.date)
			}else{
