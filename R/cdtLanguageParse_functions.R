...
cdtLanguageParse <- function(xml.path, lang = "en"){
	xml.doc <- xmlParse(xml.path)
	xml.lang <- xmlRoot(xml.doc)
	obj.lang <- NULL
	obj.lang$type <- xmlName(xml.lang)
	xml.lang <- xmlToList(xml.lang)
	free(xml.doc)

	out.lang <- lapply(seq_along(xml.lang), function(j){
		obj.list <- xml.lang[j]
		name.list <- names(obj.list)
		if(name.list == "title") ret <- get.lang.value0(obj.list, lang)
		if(name.list == "help"){
