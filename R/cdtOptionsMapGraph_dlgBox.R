...
MapGraph.MapOptions <- function(climMapOpt, parent.win = .cdtEnv$tcl$main$win){
    if(WindowsOS()){
        ...
        largeur1 <- 24
        largeur2 <- 59
        largeur3 <- .cdtEnv$tcl$fun$w.scale(33)
        largeur4 <- 50
        largeur5 <- 20
    }
...
MapGraph.GraphOptions.LineSHP <- function(shpLineOpt, parent.win = .cdtEnv$tcl$main$win){
    ...
    txt.pltLineSHP <- tklabel(frLinesProp, text = lang.dlg[['label']][['1']], anchor = 'w', justify = 'left')
...
## same as MapGraph.GraphOptions.Bar (diff: color negative and positive)
