...
getParams.RR.OutlierCheck <- function(Parameters)
{
    ...
    txt.rrmax <- tklabel(framePars, text = lang.dlg[['label']][['1']], anchor = 'e', justify = 'right')
    en.rrmax <- tkentry(framePars, width = 4, textvariable = precip.max, justify = 'left')
    txt.frpr <- tklabel(framePars, text = "", width = largeur0)
    txt.confL <- tklabel(framePars, text = lang.dlg[['label']][['2']], anchor = 'e', justify = 'right')
    en.confL <- tkentry(framePars, width = 3, textvariable = mul.sigma, justify = 'left')
    ...
    tkgrid(txt.confL, row = 0, column = 3, sticky = 'we', rowspan = 1, columnspan = 1, padx = 1, pady = 1, ipadx = 1, ipady = 1)
    tkgrid(en.confL, row = 0, column = 4, sticky = 'we', rowspan = 1, columnspan = 1, padx = 1, pady = 1, ipadx = 1, ipady = 1)

    helpWidget(en.rrmax, lang.dlg[['tooltip']][['1']], lang.dlg[['status']][['1']])
    helpWidget(en.confL, lang.dlg[['tooltip']][['2']], lang.dlg[['status']][['2']])
