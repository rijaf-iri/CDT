...
download_Elevation_Reanalysis <- function(){
    if(WindowsOS()){
        largeur0 <- 49
        largeur1 <- 27
        largeur2 <- 16
    ...
    xml.dlg <- file.path(.cdtDir$dirLocal, "languages", "cdtDownloadReanalysis_elevation_dlgBox.xml")
    ...
    CbReanalProd <- c("JRA-55", "MERRA-2", "ERA5", "ERA5-Land")
    reanalprod <- tclVar(.cdtData$GalParams$prod)

    txt.prod <- tklabel(frA0, text = lang.dlg[['label']][['0']], anchor = 'w', justify = 'left')
    cb.prod <- ttkcombobox(frA0, values = CbReanalProd, textvariable = reanalprod, justify = 'center', width = largeur2)

    tkgrid(txt.prod, row = 0, column = 0, sticky = 'we', rowspan = 1, columnspan = 1, padx = 1, pady = 1, ipadx = 1, ipady = 1)
    tkgrid(cb.prod, row = 0, column = 1, sticky = 'we', rowspan = 1, columnspan = 1, padx = 1, pady = 1, ipadx = 1, ipady = 1)
    ...
    enfile.save <- tkentry(frA2, textvariable = dir2save, width = largeur0)
    ...
    ######
    tkgrid(frA0, row = 0, column = 0, sticky = '', rowspan = 1, columnspan = 1, padx = 5, pady = 5, ipadx = 1, ipady = 1)
