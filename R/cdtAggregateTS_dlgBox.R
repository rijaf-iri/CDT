...
AggregateTS_GetInfo <- function(){
    ...
    minhour0.txtVar <- tclVar(lang.dlg[['label']][['17']])
    minhour1.txtVar <- tclVar(lang.dlg[['label']][['16']])
    ...
    if(str_trim(tclvalue(OriginData)) %in% Cbperiod0VAL[1:2]){
        ...
        if(str_trim(tclvalue(OriginData)) == Cbperiod0VAL[1]){
            ...
            tclvalue(minhour0.txtVar) <- lang.dlg[['label']][['17']]
            ...
            if(str_trim(tclvalue(ConvertData)) == Cbperiod0VAL[1]){
                if(as.numeric(str_trim(tclvalue(minhour.in))) == 5){
                    ...
                    tclvalue(minhour1.txtVar) <- lang.dlg[['label']][['17']]
                }
                ...
                if(as.numeric(str_trim(tclvalue(minhour.in))) == 30){
                    ...
                    tclvalue(minhour1.txtVar) <- lang.dlg[['label']][['16']]
                }
            }
        ...
        if(str_trim(tclvalue(OriginData)) == Cbperiod0VAL[2]){
            ...
            tclvalue(minhour0.txtVar) <- lang.dlg[['label']][['16']]
            tclvalue(minhour1.txtVar) <- lang.dlg[['label']][['16']]
    ...
    txt.shour <- tklabel(frameOH, text = lang.dlg[['label']][['18']], anchor = 'e', justify = 'right')
    ...
    helpWidget(en.shour, lang.dlg[['tooltip']][['16']], lang.dlg[['status']][['16']])
    ...
    tkbind(cb.intstep, "<<ComboboxSelected>>", function(){
      ...
      if(str_trim(tclvalue(OriginData)) == Cbperiod0VAL[3]) AGGRFUN <- c("mean", "sum", "max", "min", "count")
      tkconfigure(cb.aggfun, values = AGGRFUN)
      if(str_trim(tclvalue(aggr.fun)) == "count") tclvalue(aggr.fun) <- "sum"
      stateCount <- if(str_trim(tclvalue(aggr.fun)) == "count") "normal" else "disabled"
