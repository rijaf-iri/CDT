...
## Climatology index
## old name: 
getClimatologiesIndex <- function(...) cdt.index.Climatologies(...)
...
## old name:
getIndexSeasonVars <- function(...) cdt.index.flexseason(...)
...
cdt.index.aggregate <- function(dates,
								tstep.in = c("hourly", "daily", "pentad", "dekadal", "monthly"),
								tstep.out = c("daily", "pentad", "dekadal", "monthly", "annual", "seasonal", "roll.seas"),
								seasonLength = 3, startMonth = 1)
{
	tstep.in <- tstep.in[1]
	tstep.out <- tstep.out[1]

	if(tstep.out == "daily"){
		index <- split(seq_along(dates), substr(dates, 1, 8))
		odaty <- names(index)
		nbd0 <- rep(24, length(odaty))
	}
	...
	if(tstep.out == "pentad"){
		...
		odaty <- names(index)
		if(tstep.in == "hourly") nbd0 <- 24 * nb.Day.Of.Pentad(odaty)
		if(tstep.in == "daily") nbd0 <- nb.Day.Of.Pentad(odaty)
	}

	if(tstep.out == "dekadal"){
		yymm <- substr(dates, 1, 6)
		if(tstep.in %in% c("hourly", "daily")){
			jour <- as.numeric(substr(dates, 7, 8))
			jour <- cut(jour, c(1, 10, 20, 31), labels = FALSE, include.lowest = TRUE)
			index <- split(seq_along(dates), paste0(yymm, jour))
			odaty <- names(index)
			if(tstep.in == "hourly") nbd0 <- 24 * nb.Day.Of.Dekad(odaty)
			if(tstep.in == "daily") nbd0 <- nb.Day.Of.Dekad(odaty)
		}
		if(tstep.in == "pentad"){
	...
	if(tstep.out == "monthly"){
		...
		odaty <- names(index)
		if(tstep.in == "hourly") nbd0 <- 24 * nb.Day.Of.Month(odaty)
	...
	if(tstep.out == "annual"){
		...
		odaty <- names(index)
		if(tstep.in == "hourly") nbd0 <- 24 * nb.Day.Of.Year(odaty)
		if(tstep.in == "daily") nbd0 <- nb.Day.Of.Year(odaty)
		if(tstep.in == "pentad") nbd0 <- rep(72, length(odaty))
		if(tstep.in == "dekadal") nbd0 <- rep(36, length(odaty))
		if(tstep.in == "monthly") nbd0 <- rep(12, length(odaty))
	}

	if(tstep.out %in% c("seasonal", "roll.seas")){
		if(seasonLength < 1) return(NULL)
		...
		if(seasonLength == 1){
			if(tstep.out == "seasonal"){
				ix <- substr(odaty, 5, 6) %in% str_pad(startMonth, width = 2, pad = "0")
				index <- index[ix]
				odaty <- odaty[ix]
			}
			if(tstep.in == "hourly") nbd0 <- 24 * nb.Day.Of.Month(odaty)
			...
			ystart <- ystart[-length(ystart)]
			yend <- seq(cdates[length(cdates)], as.Date(paste0(format(cdates[length(cdates)], '%Y'), '-12-31')), "month")
			yend <- yend[-1]
			if(length(ystart) > 0) cdates <- c(ystart, cdates)
			if(length(yend) > 0) cdates <- c(cdates, yend)
			...
			idx <- relist(idrow[unlist(ij)], ij)
			idx <- lapply(idx, function(x) x[!is.na(x)])

			if(tstep.in == 'hourly') nbd0 <- sapply(ij, function(j) 24 * sum(nb.Day.Of.Month(cdates[j])))
			if(tstep.in == 'daily') nbd0 <- sapply(ij, function(j) sum(nb.Day.Of.Month(cdates[j])))
			if(tstep.in == 'pentad') nbd0 <- rep(6 * seasonLength, length(ij))
			if(tstep.in == 'dekadal') nbd0 <- rep(3 * seasonLength, length(ij))
			if(tstep.in == 'monthly') nbd0 <- rep(seasonLength, length(ij))

			cdates <- do.call(rbind, lapply(ij, function(j) cdates[j][c(1, seasonLength)]))
...
## old name: 
get.Index.DailyYears <- function(...) cdt.index.DailyYears(...)
