...
computeTvarsProcs <- function(){
	Insert.Messages.Out(paste("Compute", .cdtData$GalParams$Tstep, tolower(.cdtData$GalParams$variable), "temperature ......"))
	...
	if(.cdtData$GalParams$data.type == "cdtnetcdf"){
		...
		if(.cdtData$GalParams$variable == "Mean"){
			ncname <- "tmean"
			longname <- paste(str_to_title(.cdtData$GalParams$Tstep), "mean temperature")
		}
		if(.cdtData$GalParams$variable == "Range"){
			ncname <- "range"
			longname <- paste(str_to_title(.cdtData$GalParams$Tstep), "temperature range")
		}
