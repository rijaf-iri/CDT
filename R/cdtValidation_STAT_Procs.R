...
STAT_ValidationDataExec <- function(GeneralParameters){
	...
	if(!is.null(.cdtData$EnvData$inputInfo))
		if(isTRUE(all.equal(.cdtData$EnvData$stnData$inputInfo, inputInfo)))
			readstnData <- FALSE
	...
	if(readstnData){
		...
		if(readstnData1 | readstnData2){
			...
			outValidation <- file.path(GeneralParameters$outdir, paste0('HOVALIDATION_',
										file_path_sans_ext(GeneralParameters$STN.file1)))
		...
		.cdtData$EnvData$stnData$inputInfo <- inputInfo
	}
