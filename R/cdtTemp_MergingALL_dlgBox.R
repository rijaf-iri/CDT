...
Temp_mergeGetInfoALL <- function(){
	...
	# xml.dlg <- file.path(.cdtDir$dirLocal, "languages", "cdtTemp_MergingALL_dlgBox.xml")
	...
	frTab1 <- tkframe(conf.tab1)
		...
		infobulle(bt.DateRange, 'Set the start and end date to merge temperature data')
		status.bar.display(bt.DateRange, 'Set the start and end date to merge temperature data')
		...
		dir.TEMP <- tclVar(.cdtData$GalParams$TEMP$dir)
		...
		txt.RFE <- tklabel(frInputData, text = 'Directory containing downscaled data', anchor = 'w', justify = 'left')
		set.RFE <- ttkbutton(frInputData, text = .cdtEnv$tcl$lang$global[['button']][['5']])
		en.RFE <- tkentry(frInputData, textvariable = dir.TEMP, width = largeur2)
		...
		tkconfigure(set.RFE, command = function(){
			.cdtData$GalParams[["TEMP"]] <- getInfoNetcdfData(tt, .cdtData$GalParams[["TEMP"]],
															str_trim(tclvalue(dir.TEMP)), str_trim(tclvalue(file.period)))
		})

		tkconfigure(bt.RFE, command = function(){
			dirrfe <- tk_choose.dir(getwd(), "")
			tclvalue(dir.TEMP) <- if(!is.na(dirrfe)) dirrfe else ""
		})
		...
		infobulle(en.RFE, 'Enter the full path to the directory containing the downscaled data')
		status.bar.display(en.RFE, 'Enter the full path to the directory containing the downscaled data')
	...
	tkconfigure(bt.prm.OK, command = function(){
				...
				(str_trim(tclvalue(file.grddem)) == ""))
		{
			...
			.cdtData$GalParams$TEMP$dir <- str_trim(tclvalue(dir.TEMP))
