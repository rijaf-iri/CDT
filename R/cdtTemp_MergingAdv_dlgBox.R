...
mrgGetInfoTemp <- function(){
	...
	# xml.dlg <- file.path(.cdtDir$dirLocal, "languages", "cdtTemp_MergingAdv_dlgBox.xml")
	...
	frTab1 <- tkframe(conf.tab1)
		...
		infobulle(bt.DateRange, 'Set the start and end date to merge adjusted or downscaled data')
		status.bar.display(bt.DateRange, 'Set the start and end date to merge adjusted or downscaled data')
		...
		dir.RFE <- tclVar(.cdtData$GalParams$TEM$dir)
	...
	tkconfigure(bt.prm.OK, command = function(){
			...
			str_trim(tclvalue(LMCoef.dir)) %in% c("", "NA"))
		{
			...
			tkwait.window(tt)
		}else if((tclvalue(aspect.auxvar) == "1" |
				tclvalue(slope.auxvar) == "1" |
				tclvalue(dem.auxvar) == "1" |
				tclvalue(blankGrd) == "Use DEM") &
