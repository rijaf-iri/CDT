...
MultipleData.Plot.Map <- function(){
	...
	Xaxis <- list(relation = "same", draw = TRUE, at = pars.x$axp, labels = xylabs$xaxl, alternating = c(1, 2), tck = c(1, 1))
	Yaxis <- list(relation = "same", draw = TRUE, at = pars.y$axp, labels = xylabs$yaxl, alternating = c(1, 2), tck = c(1, 1))
	...
	Plot.Obj <- do.call(latticeExtra:::c.trellis, Plot.Obj)
	...
	#######
	par.StripText <- list(cex = 0.8, col = 'black', font = 2)
	...
	colorkey <- list(space = place, col = brks$colors, at = brks$legend.breaks$breaks,
					labels = list(labels = brks$legend.axis$labels, at = brks$legend.axis$at, cex = 0.8, col = 'black', rot = 0),
	...
	grob.Obj <- grid::textGrob(colorkey.Title, x = pars.key$x, y = pars.key$y, rot = pars.key$rot,
						...
						gp = grid::gpar(fontsize = 12, fontface = 'plain', col = "black", cex = 0.8))
