...
legendLabel.plotOnsetMaps <- function(lab.breaks, donDate, start.date){
    donDates <- format(start.date, "%Y")
    idt <- which(donDates == donDate)
    legendLabel <- format(lab.breaks + start.date[idt], '%d-%b')
    return(legendLabel)
}
