...
qcTTOutlierCheckPanelCmd <- function(){
	...
	GeneralParameters <- list(intstep = "daily", infile1 = "", infile2 = "",
							...
							params = list(
								...
								temp.max = 60, temp.min = -10, conf.lev = 99.73,
								window = 30)
							)
	...
	subfr1 <- bwTabScrollableFrame(cmd.tab1)
		...
		vartxtn <- tclVar(GeneralParameters$qc.tmax)
		input.file1 <- tclVar(GeneralParameters$infile1)
		input.file2 <- tclVar(GeneralParameters$infile2)

		txt.INfile1 <- 'File containing stations TMAX data'
		txt.INfile2 <- 'File containing stations TMIN data'
		txt.INfile1.var <- tclVar(txt.INfile1)
		txt.INfile2.var <- tclVar(txt.INfile2)
		...
		cb.infile1 <- ttkcombobox(frameInData, values = unlist(listOpenFiles), textvariable = input.file1, width = largeur1)
		...
		cb.infile2 <- ttkcombobox(frameInData, values = unlist(listOpenFiles), textvariable = input.file2, width = largeur1)
		bt.infile2 <- tkbutton(frameInData, text = "...")

		txt.vartxtn <- tklabel(frameVar, text = 'Variable to control', anchor = 'w', justify = 'left')
		rb.vartxtn1 <- tkradiobutton(frameVar, text = "TMAX", variable = vartxtn, value = "1", anchor = 'w', justify = 'left', background = 'lightblue')
		rb.vartxtn2 <- tkradiobutton(frameVar, text = "TMIN", variable = vartxtn, value = "0", anchor = 'w', justify = 'left', background = 'lightblue')
		tkgrid(txt.vartxtn, rb.vartxtn1, rb.vartxtn2)

		tkconfigure(bt.infile1, command = function(){
			dat.opfiles <- getOpenFiles(.cdtEnv$tcl$main$win)
			if(!is.null(dat.opfiles)){
				update.OpenFiles('ascii', dat.opfiles)
				listOpenFiles[[length(listOpenFiles) + 1]] <<- dat.opfiles[[1]]
				tclvalue(input.file1) <- dat.opfiles[[1]]
				lapply(list(cb.infile1, cb.infile2), tkconfigure, values = unlist(listOpenFiles))
			}
		})

		tkconfigure(bt.infile2, command = function(){
			dat.opfiles <- getOpenFiles(.cdtEnv$tcl$main$win)
			if(!is.null(dat.opfiles)){
				update.OpenFiles('ascii', dat.opfiles)
				listOpenFiles[[length(listOpenFiles) + 1]] <<- dat.opfiles[[1]]
		...
		tkgrid(frameVar, row = 0, column = 0, sticky = 'we', rowspan = 1, columnspan = 5, padx = 1, pady = 1, ipadx = 1, ipady = 1)
		tkgrid(txt.infile1, row = 1, column = 0, sticky = 'we', rowspan = 1, columnspan = 5, padx = 1, ipadx = 1)
		tkgrid(cb.infile1, row = 2, column = 0, sticky = 'we', rowspan = 1, columnspan = 4, padx = 0, ipadx = 1)
		tkgrid(bt.infile1, row = 2, column = 4, sticky = 'w', rowspan = 1, columnspan = 1, padx = 0, ipadx = 1)
		tkgrid(txt.infile2, row = 3, column = 0, sticky = 'we', rowspan = 1, columnspan = 5, padx = 1, ipadx = 1)
		tkgrid(cb.infile2, row = 4, column = 0, sticky = 'we', rowspan = 1, columnspan = 4, padx = 0, ipadx = 1)
		tkgrid(bt.infile2, row = 4, column = 4, sticky = 'w', rowspan = 1, columnspan = 1, padx = 0, ipadx = 1)

		helpWidget(cb.infile1, 'Select the file containing the maximum temperature data', 'Select the file containing the maximum temperature data')
		helpWidget(bt.infile1, 'Browse file if not listed', 'Browse file if not listed')
		helpWidget(cb.infile2, 'Select the file containing the minimum temperature data', 'Select the file containing the minimum temperature data')
		helpWidget(bt.infile2, 'Browse file if not listed', 'Browse file if not listed')

		tkbind(rb.vartxtn1, "<Button-1>", function(){
			tclvalue(txt.INfile1.var) <- txt.INfile1
			tclvalue(txt.INfile2.var) <- txt.INfile2
			helpWidget(cb.infile1, 'Select the file containing the maximum temperature data', 'Select the file containing the maximum temperature data')
			helpWidget(cb.infile2, 'Select the file containing the minimum temperature data', 'Select the file containing the minimum temperature data')
		})

		tkbind(rb.vartxtn2, "<Button-1>", function(){
			tclvalue(txt.INfile1.var) <- txt.INfile2
			tclvalue(txt.INfile2.var) <- txt.INfile1
			helpWidget(cb.infile2, 'Select the file containing the maximum temperature data', 'Select the file containing the maximum temperature data')
			helpWidget(cb.infile1, 'Select the file containing the minimum temperature data', 'Select the file containing the minimum temperature data')
		})
		...
		tkconfigure(bt.params, command = function(){
			...
			GeneralParameters[["params"]] <<- getParams.TT.OutlierCheck(Params)
		})

		tkgrid(chk.elev, row = 0, column = 0, sticky = 'we', rowspan = 1, columnspan = 2, padx = 1, pady = 1, ipadx = 1)
		tkgrid(bt.elev, row = 0, column = 2, sticky = 'we', rowspan = 1, columnspan = 1, padx = 1, pady = 1, ipadx = 1)
		tkgrid(bt.params, row = 1, column = 0, sticky = 'we', rowspan = 1, columnspan = 3, padx = 1, pady = 1, ipadx = 1)
		...
		tkgrid(bt.doQC, row = 4, column = 0, sticky = '', padx = 1, pady = 1, ipadx = 1, ipady = 1)
	...
	subfr2 <- bwTabScrollableFrame(cmd.tab2)
		...
		tkconfigure(bt.display.QC, command = function(){
			...
			tcl(table1, "tag", "configure", "ttestval", bg = "aquamarine", anchor = "c")
	...
	subfr4 <- bwTabScrollableFrame(cmd.tab4)
		...
		.cdtData$EnvData$sat$Opt <- list(
										user.colors = list(custom = TRUE, color = colorRampPalette(colors()[c(113:109, 86, 142, 144, 147)])(100)),
										user.levels = list(custom = TRUE, levels = NULL, equidist = TRUE)
									)
