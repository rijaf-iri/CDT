...
qcTTOutliersCheckProcs <- function(GeneralParameters){
	...
	if(GeneralParameters$infile1 == ""){
		Insert.Messages.Out("No stations data provided", format = TRUE)
		return(NULL)
	}
	...
	if(is.null(don2)){
		...
		Insert.Messages.Out(paste("No", ms, "data found"), format = TRUE)
		noCompare <- TRUE
	}else{
		if(!any(don$id %in% don2$id)){
			Insert.Messages.Out("TMIN and TMAX stations do not match", format = TRUE)
			noCompare <- TRUE
		}
		if(!any(don$dates %in% don2$dates)){
			Insert.Messages.Out("TMIN and TMAX dates do not overlap", format = TRUE)
			noCompare <- TRUE
		}
	}
	if(noCompare)
		Insert.Messages.Out("No comparison between TMIN and TMAX will be made")
	...
	if(!noCompare){
		...
		if(nrow(invtxtn)){
			...
			outqc.txtn$stn.val <- if(GeneralParameters$qc.tmax) tmax$data[invtxtn] else tmin$data[invtxtn]
		...
		rm(tmin, tmax, daty, idaty, id, stn.id, don2)
	}
	...
	outliers <- lapply(index.mon, function(it){
		...
		is <- which(colVars(x, na.rm = TRUE) > 0)
		if(length(is) == 0) return(NULL)
		istn <- istn[is]
		x <- x[, is, drop = FALSE]

		q50 <- colMedians(x, na.rm = TRUE)
		iqr <- colIQRs(x, na.rm = TRUE, type = 8)
	...
	rm(outliers.low, outliers.up, tmp.date.low, tmp.date.up)
	...
	if(length(voisin)){
		...
		is.parallel <- doparallel(length(STNsp) >= 50)
		...
		if(length(ret)){
			...
			rm(spatial.low, spatial.high, spatial.ok)
		}
		rm(STNid, STNsp, ret)
	}
	...
	if(!is.null(outqc)){
		...
		inom <- c("stn.id", "dates", "stn.val", "stats.tmp",
				"stats.sp", "status.tmp", "status.sp", "est.sp")
		nom <- c("STN.ID", "DATE", "STN.VAL", "STAT.TEMPORAL",
				"STAT.SPATIAL", "OUT.TEMORAL", "OUT.SPATIAL",
				"ESTIMATED.VAL", "NOT.REPLACE", "REPLACE.VAL")
		...
		rm(outqc.txtn, outqc.min, outqc.max,
			outqc.outlier.up, outqc.outlier.low, outqc.spatial,
